...
class SendBriefViewController: CustomBaseViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, SBPickerSelectorDelegate {
    
    @IBOutlet weak var peopleCard: UIView!
    ...
    @IBOutlet weak var avatarBackImage: PFImageView!
    ...
    var identifiers = ["NameCell", "ClientCell", "BriefCell", "DeliverablesCell", "PriceCell", "PostButtonCell"]
    ...
    // MARK: - UI Configure
    func configureUI() {
            self.title = "SEND BRIEF"
        
            if let userData = self.briefUserData {
                avatarBackImage.file = userData.avatarImg
                avatarBackImage.loadInBackground({ (image, error) -> Void in
                ...
                userProfLabel.text = userData.profession
                userLocationLabel.text = userData.locationName
                
                if userData.rate == -1 {
                    PNAPIClient.sharedInstance.getLatestRateFromUserId(userData.id, callback: { (rate, error) -> Void in
                        self.briefUserData!.rate = rate
                        self.rateButton.titleLabel?.text = "\(rate)"
                    })
                } else {
                    self.rateButton.titleLabel?.text = "\(self.briefUserData?.rate)"
                }
                
                if self.briefUserData?.isAvailable == true{
                    self.availableButton.imageView?.image = UIImage(named: "btn-available-polygon")
                }
                else{
                    self.availableButton.imageView?.image = UIImage(named: "btn-unavailable-polygon")
                }
                
                self.availableButton.selected = (self.briefUserData?.isAvailable)!
            } else {
                self.peopleCard.hidden = true
                self.view.sendSubviewToBack(self.peopleCard)
    ...
    func onClickPost(sender: AnyObject) {
        ...
        if isPhotoAdded == false {
            ...
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.Default) {
                ...
                NSLog("OK Pressed")
                SVProgressHUD.show()
                PNAPIClient.sharedInstance.postBriefProject((self.briefUserData?.id)!, name: self.nameTextField!.text!, client: self.clientTextField!.text!, brief: self.briefTextView!.text!, deliverables: self.deliverablesTextField!.text!, price: Double(self.priceTextField!.text!)!, dueDate: self.dueDate!, image: self.photoButton!.imageForState(.Normal)!) { (finished, error) -> Void in
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        ...
        if indexPath.row == 0 {
            ...
            postButton.titleLabel?.text = "SEND BRIEF"
