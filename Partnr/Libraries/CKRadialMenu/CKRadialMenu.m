...
- (id)initWithFrame:(CGRect)frame
{
  ...
  if (self) {
    ...
    self.centerView = [self makeDefaultMenuCenterView];
...
- (void) setCenterView:(UIView *)centerView {
  if (!centerView) {
    centerView = [self makeDefaultMenuCenterView];
  }
...
- (void) addPopoutView: (UIView *) popoutView withIndentifier: (NSString *) identifier {
  if (!popoutView){
      ...
      if ([identifier isEqualToString:@"Camera"]) {
          popoutView = [self makeDefaultMenuView];
          
          UIImageView *imageView = [[UIImageView alloc] initWithFrame:popoutView.frame];
          imageView.image = [UIImage imageNamed:@"menu-camera"];
          [popoutView addSubview:imageView];
      } else if ([identifier isEqualToString:@"Library"]) {
          popoutView = [self makeDefaultMenuView];
          
          UIImageView *imageView = [[UIImageView alloc] initWithFrame:popoutView.frame];
          imageView.image = [UIImage imageNamed:@"menu-note"];
          [popoutView addSubview:imageView];
      } else {
...
- (UIView *) makeDefaultMenuCenterView {
    UIView *view = [UIView new];
    view.backgroundColor = [UIColor clearColor];
    
    UIImageView *centerImage = [[UIImageView alloc] initWithFrame:self.bounds];
    centerImage.image = [UIImage imageNamed:@"btn-camera"];
